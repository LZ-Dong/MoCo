...
# More modifications made by Shanghai Jiaotong University.
#
...
def evaluate(args, model, tokenizer, checkpoint=None, prefix="", mode="dev"):
    ...
    for eval_task, eval_output_dir in zip(eval_task_names, eval_outputs_dirs):
        ...
        output_eval_file = os.path.join(eval_output_dir, prefix, f"eval_results_{mode}.txt")
...
def load_and_cache_examples(
    ...
    for f in features:
        ...
        nmask = np.zeros(args.max_nodes_num, dtype=int)
        nmask[:node_num] = 1
    ...
    all_nodes_index_mask = torch.tensor(np.array(all_nodes_index_mask), dtype=torch.float)
    all_node_mask = torch.tensor(np.array(all_node_mask), dtype=torch.int)
    all_adj_metric = torch.tensor(np.array(all_adj_metric), dtype=torch.float)
    all_sen2node_mask = torch.tensor(np.array(all_sen2node), dtype=torch.float)
    all_sen_mask = torch.tensor(np.array(all_sen_mask), dtype=torch.float)
    all_sen_length = torch.tensor(np.array(all_sen_length), dtype=torch.long)
...
parser.add_argument("--logging_steps", type=int, default=125, help="Interval certain steps to log.")
parser.add_argument("--save_steps", type=int, default=500, help="Interval certain steps to save checkpoint.")
...
parser.add_argument("--overwrite_cache", default=True, help="Overwrite the cached training and evaluation sets")
parser.add_argument("--seed", type=int, default=0, help="Random seed.")
parser.add_argument("--fp16", action="store_true", help="Whether to use 16-bit (mixed) precision (through NVIDIA apex) instead of 32-bit")
parser.add_argument("--fp16_opt_level", type=str, default="O1", help="For fp16: Apex AMP optimization level selected in ['O0', 'O1', 'O2', and 'O3']. See details at https://nvidia.github.io/apex/amp.html")
parser.add_argument("--local_rank", type=int, default=-1, help="For distributed training: local_rank")
parser.add_argument("--max_nodes_num", type=int, default=150, help="Maximum of number of nodes when input.")
parser.add_argument("--max_sentences", type=int, default=45, help="Maximum of number of sentences when input.")
parser.add_argument("--max_sen_replen", type=int, default=128, help="Maximum of length of sentences representation (after relu).")
parser.add_argument("--attention_maxscore", type=int, default=16, help="Weight of the max similarity score inside self-attention.")
parser.add_argument("--loss_type", default="rfcl", type=str, help="Loss Type, include: normal, scl, rfcl. rfcl is the complete version of CoCo, normal is the baseline.")
parser.add_argument("--gcn_layer", default=2, type=int, help="Number of layers of GAT, recommand 2.")
parser.add_argument("--dataset_name", default="GPT2", type=str, help="Name of the dataset, if blank will use GPT2 dataset")
parser.add_argument("--with_relation", default=2, type=int, help="number of relation in Relation-GCN, >=2 for multi-relation, and =0 for the vanilla GCN.")
...
def run(conf, data_dir=None):
    ...
    if args.do_eval and args.local_rank in [-1, 0]:
        ...
        for checkpoint in checkpoints:
            ...
            if args.loss_type == "scl":
                model = RobertaForGraphBasedSequenceClassification_CL.from_pretrained(checkpoint).to(args.device)
            elif args.loss_type == "rfcl":
                model = RobertaForGraphBasedSequenceClassification_RFCL.from_pretrained(checkpoint).to(args.device)
            elif args.loss_type =='normal':
